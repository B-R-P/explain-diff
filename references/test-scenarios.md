...
- Snippet has all 4 required sections: executive summary, impact table, file breakdown (5 `<details>`), risks aside
...
**Expected behavior:**
- Agent still produces the 4-section HTML structure
- Impact table has 1-2 rows
- Risk aside may be empty but still present with "None identified"

**Failure signals:**
- "This is too small for HTML" rationalization
- Drops back to Markdown "because it's simpler"
- Produces HTML but without `<aside class="warn">`
...
- Risk aside may be "None identified" since the change is mechanical
...
**Expected behavior:**
- Risk aside mentions the revert as a signal of churn, with a brief explanation
- Impact table and file breakdown treat the net change (not the intermediate revert)
- Executive summary does not mention the revert but risks section does

**Failure signals:**
- No mention of the revert in the risks section
- Impact table includes the revert as a distinct row (confusing for readers)

## Scenario 7: Zero-risk change (config-only, well-tested)
...
- Risks aside is present with the text "None identified"
- File breakdown has 2 `<details>` accordions

**Failure signals:**
- Omits the risks `<aside>` entirely
- Manufactures fake risks to avoid an empty section
